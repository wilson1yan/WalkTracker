...
public class WalkCanvas extends SurfaceView implements SurfaceHolder.Callback {
	...
	private void drawDistanceText(Canvas canvas) {
		...
		String text = getCorrectDecimalPlace(Calculator.convertedDistance,
				Calculator.measurementUnit) + Calculator.measurementUnit;
	...
	private void initGrad(Canvas canvas) {
		if (grad == null || gradVert == null) {
			grad = new GradientDrawable(Orientation.LEFT_RIGHT,
					new int[] { 0xff777777, 0x22ffffff, 0xff777777, 0x22ffffff,
							0xff777777 });
	...
	private double getCorrectDecimalPlace(double totalDistance,
			String measurementUnit) {
		if (measurementUnit.equalsIgnoreCase(Settings.MILE)
				|| measurementUnit.equalsIgnoreCase(Settings.KILOMETER)) {
	...
	private void drawBorders(Canvas canvas, Paint paint) {
		...
		canvas.drawLine(canvas.getWidth() - 2, 2, canvas.getWidth() - 2,
				canvas.getHeight() - 2, paint);
		canvas.drawLine(2, canvas.getHeight() - 2, canvas.getWidth() - 2,
				canvas.getHeight() - 2, paint);
