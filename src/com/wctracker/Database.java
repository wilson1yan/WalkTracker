...
/**
 * This is the main class where you can access the database and use it with the
 * DbHelper Helps with inserting and deleting
 * 
 * @author Wilson Yan
 * 
 */
public class Database {
	...
	public ArrayList<Log> getLogs() {
		...
		Cursor cursor = db.query(DbHelper.LOGS_TABLE, null, null, null, null,
				null, null);
		cursor.moveToNext();
		...
		while (!cursor.isAfterLast()) {
			...
			int caloriesBurned = cursor.getInt(cursor
					.getColumnIndex(DbHelper.CALORIES_BURNED));
			int time = cursor.getInt(cursor.getColumnIndex(DbHelper.TIME));
			String measurement = cursor.getString(cursor
					.getColumnIndex(DbHelper.MEASUREMENT));
			long id = cursor.getLong(cursor.getColumnIndex(DbHelper.ID_NUM));

			logs.add(new Log(date, time, caloriesBurned, distance, measurement,
					id));
			cursor.moveToNext();
		}
		db.close();
		...
		while (!cursor.isAfterLast()) {
			if (cursor.getLong(cursor.getColumnIndex(DbHelper.ID_NUM)) == walkId) {
				...
				String measurement = cursor.getString(cursor
						.getColumnIndex(DbHelper.MEASUREMENT));
				long id = cursor
						.getLong(cursor.getColumnIndex(DbHelper.ID_NUM));

				db.close();

				return new Log(date, time, caloriesBurned, distance,
						measurement, id);
			} else {
				cursor.moveToNext();
			}
	...
	public ArrayList<Location> getCurrentWalkPath() {
		...
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		Cursor cursor = db.query(DbHelper.WALKING_GEOPOINT, null, null, null,
				null, null, null);
		...
		while (!cursor.isAfterLast()) {
			if (cursor.getInt(cursor.getColumnIndex(DbHelper.WALK_ID)) == id) {
				double latitude = cursor.getDouble(cursor
						.getColumnIndex(DbHelper.LATITUDE));
				double longitude = cursor.getDouble(cursor
						.getColumnIndex(DbHelper.LONGITUDE));
	...
	public void deleteLog(long walkId, Context context) {
		...
		db.delete(DbHelper.WALKING_GEOPOINT, DbHelper.WALK_ID + "=" + walkId,
				null);
