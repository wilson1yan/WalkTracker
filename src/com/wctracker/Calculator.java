...
public class Calculator {
	...
	public static double totalDistance = 0, totalCalories = 0,
			convertedDistance = 0;
	...
	public Calculator(double totalDistance, double totalCalories,
			long startTime, double weight, String measurementUnit,
			String calorieType) {
		super();
		...
		Calculator.convertedDistance = (totalDistance * ((Double) METRIC_CONVERSION
				.get(measurementUnit)).doubleValue());
	}
	...
	public void calculate(double distance, double timeLength) {
		...
		if (speed <= MAX_WALKING_SPEED) {
			totalCalories += calculateCalories(distance, timeLength, weight,
					Calculator.calorieType);

		}

		convertedDistance = (totalDistance * ((Double) METRIC_CONVERSION
	...
	private double findSpeed(double distance, double hours) {
		double km = 0;
		// km =
		// (distance/(Double)METRIC_CONVERSION.get(Calculator.measurementUnit))/1000;
		km = distance / 1000;
		return km / hours;
	}

	private double calculateCalories(double distance, double timeSec,
			double weight, String calorieType) {
		...
		if (calorieType.equalsIgnoreCase("Gross")) {
			calories = (0.0215 * Math.pow(kph, 3) - 0.1765 * Math.pow(kph, 2)
					+ 0.8710 * kph + 1.4577)
					* kg * hours;
		} else {
			calories = (0.0215 * Math.pow(kph, 3) - 0.1765 * Math.pow(kph, 2) + 0.8710 * kph)
					* kg * hours;
		}
	...
	public void updateInfo(SharedPreferences sharedPreferences) {
		sharedPreferences
				.edit()
				.putInt(Settings.CURRENT_DISTANCE_KEY,
						(int) Calculator.totalDistance).commit();
		sharedPreferences
				.edit()
				.putInt(Settings.CURRENT_CALORIE_KEY,
						(int) Calculator.totalCalories).commit();
		sharedPreferences.edit().putLong(Settings.CURRENT_START_KEY, startTime)
				.commit();
	}
