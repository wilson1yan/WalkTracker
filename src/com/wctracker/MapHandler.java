...
public class MapHandler {
	...
	public MapHandler(GoogleMap map) {
		this.map = map;
		this.bitmapDescriptor = BitmapDescriptorFactory
	...
	public void updateMap(ArrayList<Location> path) {
		...
		if (path.size() >= 3) {
			addToPath(new LatLng(loc1.getLatitude(), loc1.getLongitude()),
	...
	private void addToPath(LatLng current, LatLng last) {
		getMap().addPolyline(
				new PolylineOptions().color(Color.RED).width(8).add(current)
						.add(last));
	...
	public ArrayList<PolylineOptions> findLinesWithinBounds(
		...
		for (int i = 1; i < path.size() - 1; i++) {
			...
			if (!first && next) {
				line = null;
				line = new PolylineOptions().color(Color.RED).width(8)
						.add(path.get(i)).add(path.get(i + 1));
			} else if (first && next) {
				if (line == null) {
					line = new PolylineOptions().color(Color.RED).width(8)
							.add(path.get(i)).add(path.get(i + 1));
